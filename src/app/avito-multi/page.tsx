...
function AvitoMultiComponent() {
  ...
  const [company, setCompany] = useState<string>("seltka");
  ...
  useEffect(() => {
    const qCompany = search.get("company") || undefined;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        console.error("Avito Multi load error:", e);
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1400, margin: "0 auto", fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>📊 Avito Мульти-компания (реальные данные через Supabase RPC)</h1>
      <p style={{ color: '#6c757d', marginBottom: 16 }}>
        Анализ данных по нескольким компаниям с реальными данными из Supabase.
      </p>
      ...
      <section style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <select id="company" value={company} onChange={(e) => setCompany(e.target.value)} style={{ padding: '6px 10px', border: '1px solid #e1e1e1', borderRadius: 6 }}>
